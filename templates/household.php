...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .member-list { display: grid; gap: 8px; margin: 0; padding: 0; list-style: none; }
...
<body>
    ...
    <main id="family-manager-household">
        ...
        <section class="stats" aria-label="<?php echo esc_attr__( 'Household summary', 'family-manager' ); ?>">
            <div class="stat"><strong data-stat="members">0</strong><span><?php echo esc_html__( 'Members', 'family-manager' ); ?></span></div>
        ...
        <section class="grid">
            ...
            <div data-manage hidden>
                <div class="panel">
                    <h2><?php echo esc_html__( 'Add a member', 'family-manager' ); ?></h2>
                    <p class="hint"><?php echo esc_html__( 'Creates a login for them. With an email, an existing account is linked instead.', 'family-manager' ); ?></p>
                    <form data-action="add_member">
                        ...
                        <input name="email" type="email" placeholder="<?php echo esc_attr__( 'Email (optional)', 'family-manager' ); ?>">
                        <input name="password" type="text" autocomplete="off" placeholder="<?php echo esc_attr__( 'Password (optional)', 'family-manager' ); ?>">
                        <button type="submit"><?php echo esc_html__( 'Add member', 'family-manager' ); ?></button>
                    </form>
                </div>
                <div class="panel">
                    <h2><?php echo esc_html__( 'Settings', 'family-manager' ); ?></h2>
                    <form data-action="update_household" data-settings>
                        <input name="name" required aria-label="<?php echo esc_attr__( 'Household name', 'family-manager' ); ?>">
                        <label class="check">
                            <input type="checkbox" name="rewards_enabled" value="1">
                            <span><?php echo esc_html__( 'Use points and rewards', 'family-manager' ); ?><small><?php echo esc_html__( 'Tasks earn points that members can trade for rewards. Leave this off if you just want a shared to-do list.', 'family-manager' ); ?></small></span>
                        </label>
                        <button type="submit"><?php echo esc_html__( 'Save settings', 'family-manager' ); ?></button>
                    </form>
                </div>
            </div>
        </section>
    ...
    <script>
        (function() {
            const root = document.getElementById('family-manager-household');
            ...
            function render(data) {
                ...
                root.querySelector('[data-subtitle]').textContent = data.permissions.manage
                    ? '<?php echo esc_js( __( 'You manage this household. Add members, change their roles and adjust settings here.', 'family-manager' ) ); ?>'
                    : '<?php echo esc_js( __( 'Everyone in this household and how to reach their profile.', 'family-manager' ) ); ?>';
                ...
                data.members.forEach((member) => {
                    ...
                    item.querySelector('.title').textContent = member.name + (isSelf ? ' (<?php echo esc_js( __( 'you', 'family-manager' ) ); ?>)' : '');
                    item.querySelector('.meta').textContent = [member.role_label, '@' + member.login].concat(rewards ? [member.points + ' <?php echo esc_js( __( 'pts', 'family-manager' ) ); ?>'] : []).join(' · ');
                    ...
                    if (data.permissions.manage && !isSelf) {
                        ...
                        view.href = window.familyManager.memberUrl + member.id + '/';
                        view.textContent = '<?php echo esc_js( __( 'View as', 'family-manager' ) ); ?>';
                        view.title = '<?php echo esc_js( __( 'See the dashboard as this member', 'family-manager' ) ); ?>';
                        actions.appendChild(view);

                        const select = document.createElement('select');
                        select.setAttribute('aria-label', '<?php echo esc_js( __( 'Role', 'family-manager' ) ); ?>');
            ...
            root.querySelectorAll('form[data-action]').forEach((form) => {
                form.addEventListener('submit', (event) => {
                    ...
                    save(payload);
                    if (form.dataset.action === 'add_member') {
                        form.reset();
                    } else {
            ...
            request({ family_action: 'get' }).then(render).catch((error) => status.textContent = error.message);
        })();
    </script>
