...
- **Who is where** lives at `/households/where/`: who is here right now, a day-by-day board for the next fortnight, and the handovers that follow from it. People moving together on the same day are one handover, because that is one trip. It spans homes, so it says which one it is reading from and lets you pick another.
