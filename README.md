...
- **Each member has their own view** at `/households/`: children see only the tasks and rewards assigned to them or to the whole household; organisers (parents and up) see everything and can add tasks and rewards.
- **Everyone in more than one household gets an overview** at `/households/households/`, reachable from the household count in the dashboard status line: each household with your role there, its members, how much is open, and a link to switch to it.
- **Administrators manage the household** at `/households/household/`: add members, change roles, rename the household and switch optional features on. They can also open any member's view at `/households/member/<user id>/` to check what that member sees and act on their behalf.
- **Who is where** lives at `/households/where/`: a dial showing where everyone is right now, a day-by-day board for the next fortnight, and the handovers that follow from it. People moving together on the same day are one handover, because that is one trip.
...
- **About this home** on the household page holds what people ask when they are in a house they do not live in every day: the wifi network, where the water main valve is, which day the bins go out. It belongs to the household because it is true of the place, and everyone in that household can read it — so it is not the place for anything that should not be shared with all of them.
>>>>>>> cca5a70 (Rename to Households and add a whereabouts view)
...
- Tasks and rewards are private posts tagged with a `household_member` term per user; points, rotations and their one-off changes are user meta.
